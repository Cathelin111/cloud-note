...
//修改笔记本名称
function updateBookName(notebookId,name,dom){
	$.ajax({
		url:base_path+"/notebook/updateName.do",
		type:"post",
		data:{"notebookId":notebookId,"name":name},
		dataType:"json",
		success:function(result){
			if(result.status==0){
				//重建笔记本li的<a>内容(图标+新名称+删除按钮)
				dom.children("a").html('<i class="fa fa-book" title="online" rel="tooltip-bottom"></i>'+name+'<button type="button" class="btn btn-default btn-xs btn_position btn_delete"><i class="fa fa-times"></i></button>');
				alert(result.msg);
			}else{
				alert(result.msg);
			}
...
//彻底删除笔记本
function deleteBook(notebookId,dom){
	$.ajax({
		url:base_path+"/notebook/delete.do",
		type:"post",
