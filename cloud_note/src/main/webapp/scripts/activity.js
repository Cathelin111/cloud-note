...
function getActivityList(){
	$.ajax({
		type:"post",
		url:base_path+"/activity/findActivity.do",
		dataType:"json",
		data:{},
...
function getNoteActivitys(activityId,currentPage){
	$.ajax({
		type:"post",
		url:base_path+"/activity/findNoteActivity.do",
		dataType:"json",
...
function createNoteActivity(noteId,activityId,dom){
	$.ajax({
		type:"post",
		url:base_path+"/activity/addNoteActivity.do",
		dataType:"json",
		data:{"activityId":activityId,"noteId":noteId,"userId":getCookie("userId")},
		success:function(result) {
			if(result.status==0) {
				var noteActivity = result.data;
...
function likeActivityNote(noteActivityId, dom) {
	$.ajax({
		type:"post",
		url:base_path+"/note/likeActivityNote.do",
		dataType:"json",
		data:{"noteActivityId":noteActivityId,"userId":getCookie("userId")},
		success:function(result) {
			if(result.status==0) {
				dom.remove();
			} else {
...
function up(noteActivityId, dom) {
	$.ajax({
		type:"post",
		url:base_path+"/activity/upNoteActivity.do",
		dataType:"json",
		data:{"noteActivityId":noteActivityId},
		success:function(result) {
			if(result.status==0) {
...
function down(noteActivityId, dom) {
	$.ajax({
		type:"post",
		url:base_path+"/activity/downNoteActivity.do",
		dataType:"json",
		data:{"noteActivityId":noteActivityId},
		success:function(result) {
			if(result.status==0) {
